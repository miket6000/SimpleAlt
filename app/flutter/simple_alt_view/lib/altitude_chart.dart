...
class YAxis {
  ...
  YAxis(this.recording, this.label) {
    ...
    minY = recording.values[label]!.reduce((a, b)=>(a[1] < b[1] ? a : b))[1]; 
    maxY = recording.values[label]!.reduce((a, b)=>(a[1] > b[1] ? a : b))[1]; 
    if (minY == maxY) {
      scalar = 1;
    } else {
      scalar = (maxY - minY);
    ...
    spots = [...recording.values[label]!.map((e)=>FlSpot(e[0], (e[1] - minY)/scalar))];
  }
}
